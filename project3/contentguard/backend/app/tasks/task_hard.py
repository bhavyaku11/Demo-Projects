...
from ..graders.grader_hard import HardMetrics, NuancedModerationGrader
from ..models import Action, Observation


class NuancedModerationTask:
    ...
    def __init__(self) -> None:
        data_path = Path(__file__).parent.parent / "data" / "posts_hard.json"
        with data_path.open(encoding="utf-8") as file:
            self.posts = json.load(file)
        self.grader = NuancedModerationGrader()
        self.reset()

    def reset(self) -> Observation:
        self.queue = random.sample(self.posts, min(self.MAX_STEPS, len(self.posts)))
        self.step_idx = 0
        self.metrics = HardMetrics()
    ...
    def step(self, action: Action):
        post = self.queue[self.step_idx]
        reward = self.grader.compute_reward(action, post, self.metrics)
        self.step_idx += 1
